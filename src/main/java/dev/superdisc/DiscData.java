...
public final class DiscData extends SavedData {
    ...
    static DiscData load(CompoundTag root) {
        ...
        for(Tag tag:root.getList("tracks",Tag.TAG_COMPOUND)) {
            Track track=Track.read((CompoundTag)tag); track.playing=false; track.syncing=false; data.tracks.put(track.key(),track);
        }
        return data;
    }
    @Override public CompoundTag save(CompoundTag root) {
        ...
        for(Track track:tracks.values()) {
            var t=track.tag(); t.putDouble("position",track.at(System.currentTimeMillis())); t.putBoolean("playing",false); t.putBoolean("syncing",false); list.add(t);
        }
