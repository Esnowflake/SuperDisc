...
public final class ServerPlayback {
    ...
    private static ServerLevel level(MinecraftServer server,Track t) {
        return server.getLevel(ResourceKey.create(Registries.DIMENSION,Identifier.parse(t.dimension)));
    }
    ...
    private static boolean near(ServerPlayer p,Track t) { return p.level().dimension().identifier().toString().equals(t.dimension) && p.distanceToSqr(t.pos.getX()+.5,t.pos.getY()+.5,t.pos.getZ()+.5)<=64; }
    ...
    private static boolean revision(Session s,CompoundTag t) { return dev.superdisc.Nbt.hasUuid(t,"revision") && s.track.revision.equals(dev.superdisc.Nbt.uuid(t,"revision")); }
    public static InteractionResult click(net.minecraft.world.entity.player.Player entity, net.minecraft.world.level.Level level, InteractionHand hand, BlockPos pos) {
        ...
        String key=player.level().dimension().identifier()+"/"+pos.asLong();
        ...
        if(track==null) {
            ...
            track=new Track(player.level().dimension().identifier().toString(),pos);
    ...
    public static void receive(ServerPlayer p,Net.Packet packet) throws Exception {
        ...
        if(op.equals("clock")) { var reply=new CompoundTag();reply.putLong("sent",t.getLongOr("sent", 0L));reply.putLong("server",System.currentTimeMillis());Net.toPlayer(p,"clock",reply);return; }
        if(op.equals("preferences")){protection(p,t.getBooleanOr("protected", false));return;}
        ...
        if(op.equals("beat") || op.equals("ended")) {
            if(!revision(s,t)||t.getLongOr("transport", 0L)!=track.transport||!track.owner.equals(p.getUUID())||!track.playing||!s.ready.contains(p.getUUID()))return;
            s.lastBeat=System.currentTimeMillis();
            if(op.equals("ended")){finish(server,s);return;}
            double position=t.getDoubleOr("actual", 0d);
            if(Double.isFinite(position)&&position>=0&&position<=track.duration) {
                track.position=position;track.epoch=System.currentTimeMillis()-Math.min(1000,Math.max(0,t.getIntOr("age", 0)));
                // Timing heartbeats do not dirty saved data or serialize the full manifest.
                var clock=new CompoundTag();clock.putString("dimension",track.dimension);clock.putLong("pos",track.pos.asLong());dev.superdisc.Nbt.putUuid(clock,"revision",track.revision);clock.putLong("transport",track.transport);clock.putDouble("position",track.position);clock.putLong("epoch",track.epoch);
        ...
        if(op.equals("ready") || op.equals("need") || op.equals("failed") || op.equals("received")) {
            ...
            if(op.equals("ready")) {
                double duration=t.getDoubleOr("decodedDuration", 0d);
            ...
            } else if(op.equals("received")) s.progress.put(p.getUUID(),Math.max(0,Math.min(track.size,t.getLongOr("received", 0L))));
            else {
                s.waiting.remove(p.getUUID());pause(server,s);
                for(ServerPlayer listener:server.getPlayerList().getPlayers()) if(s.audience.contains(listener.getUUID()))message(listener,p.getGameProfile().name()+" 的音频解码/同步失败，播放已暂停。");
                SuperDisc.LOG.warn("Client {} failed to prepare {}: {}",p.getUUID(),track.hash,t.getStringOr("error", ""));
            }
            return;
        }
        ...
        boolean uploading=(op.equals("chunk")&&p.getUUID().equals(s.uploader)&&s.upload!=null)
            || (op.equals("cancel_import")&&p.getUUID().equals(s.uploader)&&dev.superdisc.Nbt.hasUuid(t,"importRequest")&&dev.superdisc.Nbt.uuid(t,"importRequest").equals(s.importRequest));
        if(!uploading && (!near(p,track) || !exists(server,track) || !track.key().equals(viewers.get(p.getUUID())) || p.isSpectator()))return;
        if(op.equals("begin")) {
            if(request.size<=0||request.size>Cache.MAX_BYTES||!Double.isFinite(request.duration)||request.duration<0||request.duration>Cache.MAX_SECONDS||!dev.superdisc.Nbt.hasUuid(t,"importRequest")) {message(p,"音频限制：32 MiB、20 分钟以内。");return;}
            ...
            track.owner=p.getUUID();track.revision=dev.superdisc.Nbt.uuid(t,"importRequest");track.position=0;
            track.controller=new UUID(0,0);
            track.playing=false;track.syncing=true;s.wantPlay=false;s.syncStart=System.currentTimeMillis();s.importRequest=dev.superdisc.Nbt.uuid(t,"importRequest");
        ...
        if(op.equals("cancel_import")) {
            if(p.getUUID().equals(s.uploader)&&dev.superdisc.Nbt.hasUuid(t,"importRequest")&&dev.superdisc.Nbt.uuid(t,"importRequest").equals(s.importRequest))clear(server,s);
            return;
        }
        if(!revision(s,t))return;
        if(op.equals("volume_watch")) {
            if(t.getBooleanOr("watch", false)&&track.controller.equals(p.getUUID())){volumeViewers.add(p.getUUID());lastRoster.remove(p.getUUID());roster(p,s);}
            else{volumeViewers.remove(p.getUUID());lastRoster.remove(p.getUUID());}return;
        }
        if(op.equals("protect")){protection(p,t.getBooleanOr("protected", false));personal(p,s);refreshRosters(server,s);return;}
        if(op.equals("volume")) {
            UUID target=dev.superdisc.Nbt.hasUuid(t,"target")?dev.superdisc.Nbt.uuid(t,"target"):p.getUUID();
            var listener=server.getPlayerList().getPlayer(target);float value=t.getFloatOr("volume", 0f);
        ...
        if(op.equals("chunk")) {
            ...
            try {
                s.upload.append(t.getLongOr("offset", 0L),packet.bytes());s.lastUpload=System.currentTimeMillis();
            ...
            if(op.equals("seek")) {
                double at=t.getDoubleOr("seek", 0d);if(!Double.isFinite(at))return;
    ...
    private static Set<UUID> audience(MinecraftServer server,Track track) {
        ...
        for(ServerPlayer p:server.getPlayerList().getPlayers()) {
            if(p.getUUID().equals(track.owner) || p.getUUID().equals(track.controller) || (p.level().dimension().identifier().toString().equals(track.dimension) && p.distanceToSqr(track.pos.getX()+.5,track.pos.getY()+.5,track.pos.getZ()+.5)<AUDIENCE_RANGE*AUDIENCE_RANGE)) result.add(p.getUUID());
        }
        return result;
    }
    ...
    private static void roster(ServerPlayer p,Session s){
        ...
        for(var player:p.level().getServer().getPlayerList().getPlayers()){
            var row=new CompoundTag();dev.superdisc.Nbt.putUuid(row,"id",player.getUUID());row.putString("name",player.getGameProfile().name());
