...
public final class VolumeScreen extends Screen {
    ...
    @Override public void tick(){
        if(ClientPlayback.get(key)==null){minecraft.gui.setScreen(null);return;}
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor g,int mx,int my,float partial){
        ...
        for(int i=offset;i<Math.min(list.size(),offset+capacity);i++){
            ...
            PlayerFaceExtractor.extractRenderState(g,info==null?DefaultPlayerSkin.get(entry.id()):info.getSkin(),left+17,y+10,28);
            g.text(font,font.plainSubstrByWidth(entry.name(),Math.max(40,w-195)),left+55,y+5,0xFFF2F5FA);
            String badge=entry.locked()?"已保护":"允许调整";
            g.text(font,badge,left+w-18-font.width(badge),y+5,entry.locked()?0xFF94A5B5:0xFF83DCC0);
        }
        if(list.isEmpty())g.centeredText(font,"正在读取在线玩家…",left+w/2,top+92,0xFFB9C9D8);
        ...
        g.text(font,footer,left+w-12-font.width(footer),top+h-22,0xFF98ADBE);
        super.extractRenderState(g,mx,my,partial);
    }
    @Override public void onClose(){returning=true;ClientPlayback.watchVolumes(key,false);minecraft.gui.setScreen(new DiscScreen(key));}
    ...
    private final class PlayerSlider extends AbstractSliderButton {
        ...
        @Override public void onClick(net.minecraft.client.input.MouseButtonEvent event,boolean doubleClick){dragging=true;super.onClick(event,doubleClick);}
        @Override public void onRelease(net.minecraft.client.input.MouseButtonEvent event){super.onRelease(event);dragging=false;send();}
        @Override public boolean keyPressed(net.minecraft.client.input.KeyEvent event){boolean changed=super.keyPressed(event);if(changed)send();return changed;}
    }
}
