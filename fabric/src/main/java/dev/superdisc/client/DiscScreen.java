...
public final class DiscScreen extends Screen {
    ...
    @Override protected void init(){
        ...
        manage=button("多人音量管理…",left+10,top+211,(w-24)/2,()->{navigating=true;minecraft.gui.setScreen(new VolumeScreen(key));});
    ...
    @Override public void tick(){
        var l=ClientPlayback.get(key);if(l==null){onClose();return;}
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor g,int mouseX,int mouseY,float partial){
        ...
        var l=ClientPlayback.get(key);if(l!=null){
            Track t=l.track;boolean owner=minecraft.player!=null&&t.owner.equals(minecraft.player.getUUID());
            g.text(font,format(displayedTime)+" / "+format(t.duration)+(owner?"  · 可调整进度":"  · 仅导入者可调整进度"),left+10,top+100,0xFFCCCCCC);
            g.text(font,"我的音量 0–400% · "+(ClientPlayback.distortionProtection?"峰值保护已开启":"保护关闭，高增益可能破音"),left+10,top+138,ClientPlayback.distortionProtection?0xFFCCCCCC:0xFFFFB66B);
            String status=t.syncing?"等待所有收听者同步完成":t.playing?"正在播放":ClientPlayback.importing(key)?"正在校验文件并自动同步…":l.status;
            g.text(font,font.plainSubstrByWidth(status,w-20),left+10,top+244,0xFF88DDCC);
        }
    ...
    private static final class Slider extends AbstractSliderButton {
        ...
        @Override public void onRelease(net.minecraft.client.input.MouseButtonEvent event){super.onRelease(event);dragging=false;commit.accept(value);}
        @Override public boolean keyPressed(net.minecraft.client.input.KeyEvent event){boolean result=super.keyPressed(event);if(result)commit.accept(value);return result;}
