...
public final class ClientPlayback {
    ...
    public static void setVolume(String key,UUID target,double value){Local l=get(key);if(l!=null){var tag=l.track.tag();dev.superdisc.Nbt.putUuid(tag,"target",target);tag.putFloat("volume",(float)value);Net.toServer("volume",tag);}}
    ...
    public static void packet(Net.Packet packet) {
        ...
        if(op.equals("clock")){long now=System.currentTimeMillis();rtt=Math.max(0,now-tag.getLongOr("sent", 0L));clockOffset=tag.getLongOr("server", 0L)+rtt/2-now;return;}
        if(op.equals("message")){notice(tag.getStringOr("text", ""));return;}
        Track track=Track.read(tag);String key=track.key();Local l=locals.get(key);
        if(op.equals("personal")){personalVolumes.put(key,tag.getFloatOr("personalVolume", 0f));protectVolume=tag.getBooleanOr("protected", false);return;}
        if(op.equals("volumes")){
            List<Listener> entries=new ArrayList<>();var rows=tag.getListOrEmpty("players");
            for(int i=0;i<rows.size();i++){var row=rows.getCompoundOrEmpty(i);entries.add(new Listener(dev.superdisc.Nbt.uuid(row,"id"),row.getStringOr("name", ""),row.getFloatOr("volume", 0f),row.getBooleanOr("protected", false)));}
        ...
        if(op.equals("remove")){ImportJob job=imports.remove(key);if(job!=null)job.work.cancel();if(l!=null)l.close();locals.remove(key);localPaths.remove(key);if(MC.gui.screen() instanceof DiscScreen screen && screen.key.equals(key))MC.gui.setScreen(null);return;}
        ...
        if(op.equals("open")){update(track);MC.gui.setScreen(new DiscScreen(key));return;}
        ...
        if(op.equals("progress")) {
            if(l!=null)l.status=tag.getStringOr("phase", "")+" "+Math.min(100,100*tag.getLongOr("done", 0L)/Math.max(1,tag.getLongOr("total", 0L)))+"%";
            if(notifications)progress("[Super Disc] "+track.name+" — "+(l==null?tag.getStringOr("phase", ""):l.status));return;
        }
        ...
        if(op.equals("upload_ack")) {
            if(l!=null&&l.track.revision.equals(track.revision)&&l.uploadOffset>=0){l.uploadOffset=tag.getLongOr("offset", 0L);if(l.uploadOffset<track.size)sendUpload(l);}return;
        }
        ...
        if(op.equals("download")) {
            ...
            IO.execute(()->{try{
                current.work.check();incoming.append(tag.getLongOr("offset", 0L),packet.bytes());
    ...
    public static void importFile(String key,Path source,boolean autoplay) {
        ...
        IO.execute(()->{try{
            ...
            MC.execute(()->{if(generation!=connection||imports.get(key)!=job||job.work.cancelled())return;localPaths.put(key,real);
                ...
                var tag=meta.tag();dev.superdisc.Nbt.putUuid(tag,"importRequest",job.id);job.sent=true;Net.toServer("begin",tag);
    ...
    public static void clear(String key) {
        Local l=locals.get(key);if(l==null)return;
        ImportJob job=imports.remove(key);if(job!=null){job.work.cancel();cancelledRevisions.add(job.id);if(job.sent){var tag=l.track.tag();dev.superdisc.Nbt.putUuid(tag,"importRequest",job.id);Net.toServer("cancel_import",tag);}}
    ...
    public static void tick() {
        ...
        for(Local l:new ArrayList<>(locals.values())) {
            ...
            boolean sameWorld=MC.level.dimension().identifier().toString().equals(t.dimension);
    ...
    public static void notice(String message){if(MC.player!=null)MC.gui.hud.getChat().addClientSystemMessage(Component.literal("[Super Disc] "+message));}
    private static void progress(String text){
        ...
        MC.gui.hud.getChat().addPlayerMessage(Component.literal(text),progressSignature,null);
    }
    private static void replacePreviousProgress(){
        ...
        try{
            ...
            for(var field:net.minecraft.client.gui.components.ChatComponent.class.getDeclaredFields())if(List.class.isAssignableFrom(field.getType())){
                ...
                if(object instanceof List<?> list && !list.isEmpty() && list.get(0) instanceof net.minecraft.client.multiplayer.chat.GuiMessage){
                    list.removeIf(m->m instanceof net.minecraft.client.multiplayer.chat.GuiMessage message && progressSignature.equals(message.signature()));chat.rescaleChat();break;
                }
            }
