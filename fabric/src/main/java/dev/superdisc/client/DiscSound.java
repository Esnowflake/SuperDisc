...
public final class DiscSound extends AbstractTickableSoundInstance implements net.fabricmc.fabric.api.client.sound.v1.FabricSoundInstance {
    ...
    public DiscSound(Track track,AudioDecoder.Prepared pcm,double position,float initialVolume) {
        super(net.minecraft.sounds.SoundEvent.createVariableRangeEvent(Identifier.fromNamespaceAndPath(SuperDisc.ID,"audio")),SoundSource.RECORDS,RandomSource.create());
    ...
    @Override public WeighedSoundEvents resolve(SoundManager manager) {
        ...
        sound=new Sound(Identifier.fromNamespaceAndPath(SuperDisc.ID,"audio"),ConstantFloat.of(1),ConstantFloat.of(1),1,Sound.Type.FILE,true,false,16);
        var events=new WeighedSoundEvents(identifier,null);events.addSound(sound);return events;
    }
    @Override public CompletableFuture<AudioStream> getAudioStream(SoundBufferLibrary library,Identifier id,boolean loop) {
    ...
    public interface BoundStream extends AudioStream { void bind(Channel channel); }
    private final class PcmStream implements BoundStream {
        @Override public void bind(Channel channel) { DiscSound.this.bind(channel); }
